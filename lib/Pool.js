...
class Pool extends EventEmitter {
  ...
  _dispense () {
     ...
     */
    const numWaitingClients = this._waitingClients.length
  ...
  acquire (priority) {
    const executor = (resolve, reject) => {
      ...
      if (this._draining) {
        return reject(new Error('pool is draining and cannot accept work'))
      }

      // TODO: should we defer this check till after this event loop incase "the situation" changes in the meantime
      if (this._config.maxWaitingClients !== undefined && this._waitingClients.length >= this._config.maxWaitingClients) {
        return reject(new Error('max waitingClients count exceeded'))
      }
  ...
  drain () {
    const executor = (resolve, reject) => {
      ...
      const check = () => {
        if (this._waitingClients.length > 0) {
  ...
  waitingClientsCount () {
    return this._waitingClients.length
  }
