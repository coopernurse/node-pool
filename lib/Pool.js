...
class Pool extends EventEmitter {
  ...
  constructor (factory, config) {
     ...
     * @type {Array}
     */
    this._factoryResourceRequests = new Set()
  ...
  _testOnBorrow () {
    ...
    const testPromise = this._factory.validate(pooledResource.obj)

    testPromise.then(this._createValidationCallback(pooledResource))

    return true
  }

  _createValidationCallback (pooledResource) {
    return (isValid) => {
  ...
  _dispense () {
    ...
    const potentiallyAllocableResources = this._availableObjects.length +
      ...
      this._factoryResourceRequests.size
    ...
    if (resourceShortfall > 0) {
      ...
      const spareResourceCapacity = this._config.max - (this._allObjects.size() + this._factoryResourceRequests.size)
  ...
  _createResource () {
    ...
    const factoryResourceRequest = new ResourceRequest(this._createFactoryCallback())
    this._factoryResourceRequests.add(factoryResourceRequest)

    // Glue to from callback -> promishish -> callback
    this._factory.create((err, resource) => {
      // Remove factoryResourceRequest list
      this._factoryResourceRequests.delete(factoryResourceRequest)
  ...
  acquire (priority) {
    ...
    const executor = (resolve, reject) => {
      const callback = function (err, resource) {
        if (err) {
          return reject(err)
        }
        resolve(resource)
      ...
      const request = new ResourceRequest(callback, this._config.acquireTimeoutMillis)
  ...
  destroyAllNow (callback) {
    ...
    this._availableObjects = []
    let pooledResource = willDie.shift()
