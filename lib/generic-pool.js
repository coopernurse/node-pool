...
function Pool (factory) {
  ...
  if (factory.acquireTimeout) {
    factory.acquireTimeout = parseInt(factory.acquireTimeout, 10)
  }
...
Pool.prototype.destroy = function destroy (resource) {
  ...
  function _filterObjWithTimeout (objWithTimeout) {
    return (objWithTimeout.obj !== resource)
  }

  function _filterObjInUser (objInUse) {
    return (objInUse !== resource)
  }
}
...
Pool.prototype._dispense = function dispense () {
  ...
  function _asyncValidationIterator (next) {
    ...
    self._factory.validateAsync(objWithTimeout.obj, _validatationCallback)
    ...
    function _validatationCallback (isValid) {
      if (!isValid) {
        ...
        resourceRequest.fulfill(err, objWithTimeout.obj)
      }
    }
  }
  ...
  if (waitingCount > 0) {
    ...
    while (_whileCondition()) {
      ...
      return resourceRequest.fulfill(err, objWithTimeout.obj)
    }
...
Pool.prototype._createResource = function _createResource () {
  ...
  function _factoryCallback () {
    var err, obj
    var resourceRequest = self._waitingClients.dequeue()
    ...
    if (err) {
      ...
      if (resourceRequest) {
        resourceRequest.fulfill(err, obj)
      } else {
        self.release(obj)
      }
    }
...
Pool.prototype.acquire = function acquire (callback, priority) {
  ...
  var request = new ResourceRequest(callback, this._factory.acquireTimeout)
...
Pool.prototype.release = function release (resource) {
  ...
  var objWithTimeout = { obj: resource, timeout: (new Date().getTime() + this._factory.idleTimeoutMillis) }
 ...
 * @deprecated
 */
Pool.prototype.returnToPool = function returnToPool (resource) {
...
Pool.prototype.destroyAllNow = function destroyAllNow (callback) {
  ...
  this._availableObjects = []
  var objWithTimeout = willDie.shift()
  while (objWithTimeout !== null && objWithTimeout !== undefined) {
    this.destroy(objWithTimeout.obj)
    objWithTimeout = willDie.shift()
  }
  this._removeIdleScheduled = false
