var PriorityQueue = function(size, objTimeoutMillis) {
  ...
  me.enqueue = function (obj, priority) {
    ...
    slots[priority].push({ obj: obj, timeout: (new Date().getTime() + objTimeoutMillis) });
  };

  me.dequeue = function (callback) {
    var elem = null, i, sl = slots.length;
  ...
  // dequeues a lost of elements that timed out
  me.dequeueTimedout = function (callback) {
    var objs = [], i, j, slots_i, sl = slots.length, now = new Date().getTime();

    // Clear cache for total.
    total = null;
    for (i = 0; i < sl; i += 1) {
      slots_i = slots[i];
      while(slots_i.length && slots_i[0].timeout < now) {
        objs.push(slots_i.shift().obj);
      }
    }
    return objs;
  };
 ...
 *   And idle item is that is not acquired yet. Waiting items doesn't count here.
  * @param {Number} [factory.acquireTimeoutMillis = 10000]
 *   Timeout after acquire has to return a client.
 *   If not the callback will be called with an error.
...
exports.Pool = function (factory) {
  var me = {},
      ...
      acquireTimeoutMillis = factory.acquireTimeoutMillis || 10000,
      reapInterval = factory.reapIntervalMillis || 1000,
      refreshIdle = ('refreshIdle' in factory) ? factory.refreshIdle : true,
      availableObjects = [],
      waitingClients = new PriorityQueue(factory.priorityRange || 1, acquireTimeoutMillis),
      resources = [],
      removeIdleScheduled = false,
      removeIdleTimer = null,
      removeTimedoutWaitingScheduled = false,
      removeTimedoutWaitingTimer = null,
      draining = false,
  ...
  me.destroy = function(obj) {
    resources = resources.filter(function(objWithTimeout) {
  ...
  function removeIdle() {
    ...
    for (i = 0, al = availableObjects.length; i < al && (refreshIdle || (resources.length - factory.min > toRemove.length)); i += 1) {
  ...
  function removeTimedoutWaiting() {
    removeTimedoutWaitingScheduled = false;

    var timedoutClients = waitingClients.dequeueTimedout();
    if (timedoutClients.length) {
      log("removeTimedoutWaiting() acquire timed out for " + timedoutClients.length + " clients", 'warn');
      for (var i = timedoutClients.length - 1; i >= 0; i--) {
        timedoutClients[i](new Error("Acquire timeout"));
      }
    }

    scheduleRemoveTimedoutWaiting();
  }
   ...
   * More schedules cannot run concurrently.
   */
  function scheduleRemoveTimedoutWaiting() {
    if (!removeTimedoutWaitingScheduled) {
      removeTimedoutWaitingScheduled = true;
      removeTimedoutWaitingTimer = setTimeout(removeTimedoutWaiting, reapInterval);
    }
  }
  ...
  function dispense() {
    ...
    if (waitingCount > 0) {
      ...
      if (resources.length < factory.max) {
        createResource();
      }
    }
  }
  ...
  function createResource() {
    ...
    log("createResource() - creating obj - #resources=" + resources.length + " min=" + factory.min + " max=" + factory.max, 'verbose');
    factory.create(function () {
      ...
      if (err) {
        resources = resources.filter(function(compare_resource) {
          return (compare_resource !== resource);
        });
        if (clientCb) {
          clientCb(err, obj);
        }
  ...
  function ensureMinimum() {
    var i, diff;
    if (!draining && (resources.length < factory.min)) {
      diff = factory.min - resources.length;
      for (i = 0; i < diff; i++) {
        createResource();
      }
    }
  ...
  me.acquire = function (callback, priority) {
    ...
    dispense();
    scheduleRemoveTimedoutWaiting();
    return (resources.length < factory.max);
  };
  ...
  me.drain = function(callback) {
    ...
    var check = function() {
      ...
      } else if (availableObjects.length != resources.length) {
  ...
  me.destroyAllNow = function(callback) {
    ...
    clearTimeout(removeTimedoutWaitingTimer);
    if (callback) {
      callback();
    }
  };
