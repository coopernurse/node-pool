...
Pool.prototype.destroy = function destroy (resource) {
  ...
  this._allObjects = this._allObjects.filter(_filterPooledResource)
...
Pool.prototype._dispense = function dispense () {
  ...
  function _whileCondition () {
    return self._availableObjects.length > 0 && self._waitingClients.size() > 0
  }
  ...
  if (waitingCount > 0) {
    if (this._factory.validateAsync) {
      doWhileAsync(_whileCondition, this._asyncValidationIterator.bind(this), _topUpResources)
      return
    }
    ...
    while (_whileCondition()) {
      ...
      pooledResource = this._availableObjects.shift()
...
Pool.prototype._asyncValidationIterator = function(next) {
    var self = this
    this._log('dispense() - reusing obj', 'verbose')
    var pooledResource = this._availableObjects.shift()

    this._factory.validateAsync(pooledResource.obj, _validatationCallback)
    ...
    function _validatationCallback (isValid) {
      ...
      resourceRequest = self._getResourceRequest()

      if(resourceRequest === null) {
        self._addPooledResourceToAvailableObjects(pooledResource)
        return next()
      }
...
Pool.prototype._createResource = function _createResource () {
  ...
  function _factoryCallback () {
    var err, obj, pooledResource, resourceRequest
    ...
    self._allObjects.push(pooledResource)
    resourceRequest = self._getResourceRequest()
...
/**
 * Gets the next untimed-out Client ResourceRequest and discards
 * any timed-out ones it finds. Returns null if no valid waiting client
 * @return {[type]} [description]
 */
Pool.prototype._getResourceRequest = function() {
  var resourceRequest = null
  var uncheckedResourceRequest = null
  while (this._waitingClients.size() > 0 && resourceRequest === null) {
    uncheckedResourceRequest = this._waitingClients.dequeue()
...
Pool.prototype.acquire = function acquire (callback, priority) {
  ...
  if (this._factory.maxWaitingClients !== undefined && this._waitingClients.size() >= this._factory.maxWaitingClients) {
    ...
    // value
    // FIXME: we should defer this check till after this event loop incase "the situation" changes in the meantime
...
Pool.prototype.release = function release (resource) {
  ...
  var index = this._allObjects.reduce(_findIndex, -1)
  if (index < 0) {
    this._log('attempt to release an invalid resource: ' + (new Error().stack), 'error')
    return
  }

  var pooledResource = this._allObjects[index]
...
Pool.prototype.inUseObjectsCount = function inUseObjectsCount () {
  return this._allObjects.length - this._availableObjects.length
}
