 ...
 *   Indicates whether the validate is asynchronous. If so, it will be called
 *   with a second argument being a callback function that received a single
 *   argument indicating whether or not the item is valid.
    ...
    if (waitingCount > 0) {
      ...
      if ( factory.validateAsync ) {
        doWhileAsync( function() {
          ...
          factory.validate( objWithTimeout.obj, function( valid ) {
            if ( !valid ) {
